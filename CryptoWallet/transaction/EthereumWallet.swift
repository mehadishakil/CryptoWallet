...
class EthereumWallet: ObservableObject {
    ...
    @Published var fetchedTransactions: [TransactionItem] = []
    
    private var privateKey: Data?
    private let rpcURL = "https://sepolia.infura.io/v3/5c13cec41a9d4475bdd2c744a636a822"
    private let etherscanApiKey = "EM9MKXCQRUMVU37HKXFCU8679QNP4PWHVQ"
    
    init() {
        if let savedAddress = UserDefaults.standard.string(forKey: "walletAddress") {
                    self.address = savedAddress
                    // Load the private key if an address is found
                    if let savedPrivateKeyData = UserDefaults.standard.data(forKey: "privateKey") {
                        self.privateKey = savedPrivateKeyData
                        self.privateKeyHex = savedPrivateKeyData.toHexString()
                        print("Wallet initialized with saved address and private key.")
                    } else {
                        print("Found saved address but no private key. This might be an issue.")
                    }
                    getBalance()
                }
        }
    ...
    // New function to fetch transactions from Etherscan
        func fetchEtherscanTransactions() async {
            guard !address.isEmpty else {
                print("Cannot fetch transactions: Wallet address is empty.")
                return
            }
            guard !etherscanApiKey.isEmpty && etherscanApiKey != "YOUR_ETHERSCAN_API_KEY" else {
                print("Etherscan API Key is not set. Please replace 'YOUR_ETHERSCAN_API_KEY' with your actual key.")
                // Set error message for UI if needed
                await MainActor.run {
                    self.errorMessage = "Etherscan API Key not configured."
                }
                return
            }

            await MainActor.run {
                self.isLoading = true // Indicate loading for transactions
                self.errorMessage = "" // Clear previous errors
            }

            let urlString = "https://api-sepolia.etherscan.io/api?module=account&action=txlist&address=\(address)&startblock=0&endblock=99999999&page=1&offset=50&sort=desc&apikey=\(etherscanApiKey)"
            
            guard let url = URL(string: urlString) else {
                await MainActor.run {
                    self.errorMessage = "Invalid Etherscan URL."
                    self.isLoading = false
                }
                return
            }
            ...
            do {
                ...
                // Corrected: HTTPURLURLResponse -> HTTPURLResponse
                guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
                    throw WalletError.networkError
                }

                let decodedResponse = try JSONDecoder().decode(EtherscanResponse.self, from: data)

                var newTransactions: [TransactionItem] = []
                for tx in decodedResponse.result {
                    let type: TransactionType
                    let source: String
                    let transactionAmount: String

                    // Determine transaction type (sent, received, denied)
                    if tx.isError == "1" || tx.txreceipt_status == "0" {
                        type = .denied
                        source = "Failed Transaction"
                    } else if tx.from.lowercased() == self.address.lowercased() {
                        type = .sent
                        source = tx.to
                    } else { // tx.to.lowercased() == self.address.lowercased()
                        type = .received
                        source = tx.from
                    }

                    // Convert Wei to ETH
                    if let valueWei = BigUInt(tx.value) {
                        let ethValue = Double(valueWei) / pow(10, 18)
                        transactionAmount = String(format: "%.4f", ethValue)
                    } else {
                        transactionAmount = "0"
                    }
                    ...
                    newTransactions.append(TransactionItem(
                        id: UUID(uuidString: tx.hash) ?? UUID(), // Use hash as a UUID, create if invalid
                        type: type,
                ...
                await MainActor.run {
                    // Ensure no duplicates if called multiple times, or decide on a strategy
                    self.fetchedTransactions = newTransactions
                    self.isLoading = false
                }
                ...
                print("Error fetching Etherscan transactions: \(error)")
            }
        }
}
...
struct EtherscanTransaction: Codable, Identifiable {
    var id: String { hash } // Use hash as ID for Identifiable
    ...
    let gas: String
    let gasPrice: String
    let isError: String // "0" for success, "1" for failure
    let txreceipt_status: String // "1" for success, "0" for failure
    let input: String
    ...
    let methodId: String
    let functionName: String? // Optional, might not always be present
}
